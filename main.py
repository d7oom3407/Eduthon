...
def gemini(input, df=None, num_team=5, history=''):
    genai.configure(api_key="AIzaSyC-1ukbiiwLbI2WXzXYdKI0QiNGOHro3ww")
...
# df.to_excel('new_students_data.xlsx', index=False)
...
if st.button('اسأل عن الطلاب'):
    user_input = st.text_input('ماهو سؤالك؟')
    if st.button('إرسال'):
        res = gemini(user_input, history=chat_history)
        st.write(res)
    
if st.button('اسأل عن تقسيم فرق المشاريع'):
    user_input = st.text_input('ماهو سؤالك؟')
    if st.button('إرسال'):
